...
class DrilldownGenerator:
    ...
    def _generate_for_pattern(self, pattern: Pattern) -> List[Question]:
        """Génère les questions pour un pattern spécifique"""
        questions = []

        # Questions basées sur les suggestions du pattern
        for i, suggestion in enumerate(pattern.suggested_questions[:3]):
            q_type = self._infer_question_type(suggestion)
            questions.append(Question(
                ...
                priority=self._calculate_priority(pattern, i)
            ))
    ...
    def _infer_question_type(self, text: str) -> QuestionType:
        ...
        elif any(w in text_lower for w in ['contrepartie', 'fournisseur', 'client']):
